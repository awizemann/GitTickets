...
describe("ReportRequestSchema (cloudflare)", () => {
  it("accepts a valid payload", () => {
    expect(() => ReportRequestSchema.parse(validReport)).not.toThrow();
  });

  it("rejects wrong schemaVersion", () => {
    expect(() => ReportRequestSchema.parse({ ...validReport, schemaVersion: 2 })).toThrow();
  });
...
describe("MyIssuesRequestSchema (cloudflare)", () => {
  it("accepts a valid payload", () => {
...
describe("MIME whitelist parity", () => {
  it("matches the Vercel whitelist", () => {
    for (const m of [
      "image/png",
      "image/jpeg",
      "image/gif",
