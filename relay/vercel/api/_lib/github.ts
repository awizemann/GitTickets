/**
 * Thin GitHub REST API client. Two endpoints only:
 *  - POST /repos/{owner}/{repo}/issues
 *  - GET  /repos/{owner}/{repo}/issues?labels=gittickets&state=all
 */
...
/** Pages through up to 5 pages × 100 issues for the configured label. */
export async function listLabeledIssues(
  ...
  for (let page = 1; page <= 5; page += 1) {
    ...
    for (const item of items) {
      if (item.pull_request) continue; // GitHub returns PRs in the issues endpoint
      all.push({
        number: item.number,
...
export async function latestComment(args: {
  ...
  const fetchFn = args.fetchFn ?? fetch;
  // sort=created&direction=desc&per_page=1 — minimum-cost lookup.
  const url =
    `https://api.github.com/repos/${encodeURIComponent(args.owner)}/${encodeURIComponent(args.repo)}/issues/${args.issueNumber}/comments` +
