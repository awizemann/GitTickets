...
/// ``GitTickets/submit(_:)`` can render ``text`` to the user and pass the
/// same string back as ``Report/diagnosticsBlob`` — preserving the
/// "what user sees == what gets sent" invariant.
public struct DiagnosticsBlob: Sendable, Hashable {
    ...
    public let sections: [(key: String, value: String)]

    public init(text: String, sections: [(key: String, value: String)]) {
        self.text = text
        self.sections = sections
    }

    public static func == (lhs: DiagnosticsBlob, rhs: DiagnosticsBlob) -> Bool {
